...
// ── Single Q&A Card with reveal animation ──
const QuizCard = ({ item, index, isOpen, onToggle }) => {
    return (
        <div
        ...
        >
            ...
            >
                {/* Number / Icon */}
                <span
                    className={`shrink-0 w-9 h-9 rounded-xl flex items-center justify-center text-sm font-extrabold transition-colors duration-300 ${
                        isOpen
                ...
                <div className="flex-1 min-w-0 pt-1">
                    <p
                        className={`text-sm sm:text-base font-semibold leading-snug transition-colors duration-200 ${
                            isOpen ? "text-[#C8A45A]" : "text-[#0B1B31] group-hover:text-[#C8A45A]"
                        }`}
                    >
                        {item.question}
                    </p>
                    ...
                    <div className="flex gap-3 bg-gradient-to-br from-[#FFFDF9] to-[#F9F5EC] border border-[#C8A45A]/15 rounded-2xl px-4 sm:px-5 py-4">
                        <div className="shrink-0 mt-0.5">
                            <div className="w-7 h-7 rounded-lg bg-[#C8A45A]/15 flex items-center justify-center">
                                <Lightbulb size={14} className="text-[#C8A45A]" />
                            </div>
                        </div>
                        <div>
            ...
            <div className="max-w-3xl mx-auto">
                        ...
                        <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0 flex-nowrap sm:flex-wrap">
                            {categories.map((cat) => (
                                <button
                                    ...
                                    className={`shrink-0 px-3 py-2 rounded-xl text-xs font-semibold border transition-all duration-200 ${
                                        activeCategory === cat
                                ...
                                >
                                    {cat === "All" && <Layers size={11} className="inline mr-1 -mt-0.5" />}
                                    {cat}
                                </button>
                            ))}
                        </div>
                ...
                {(search || activeCategory !== "All") && (
                    <div className="flex items-center justify-between mb-4">
                        <p className="text-xs text-gray-400">
                            Showing <span className="font-semibold text-[#0B1B31]">{filtered.length}</span> result{filtered.length !== 1 ? "s" : ""}
                        </p>
                        <button
                            onClick={clearSearch}
                ...
                {!loading && items.length === 0 && (
                    <div className="text-center py-20 bg-gray-50 rounded-3xl shadow-sm border border-gray-100">
                        <div className="w-16 h-16 mx-auto rounded-2xl bg-[#F3EFE6] flex items-center justify-center mb-4">
                            <MessageSquare size={28} className="text-[#C8A45A]" />
                        </div>
                        <h2 className="text-xl font-semibold text-[#0B1B31] mb-2">No Quizzes Yet</h2>
                        <p className="text-gray-400 text-sm">Check back soon — new questions are coming!</p>
                    </div>
                )}
                ...
                {!loading && items.length > 0 && filtered.length === 0 && (
                    <div className="text-center py-12 bg-gray-50 rounded-2xl border border-gray-100">
                        ...
                        <button onClick={clearSearch} className="mt-2 text-[#C8A45A] text-xs hover:underline font-medium">
                            Clear filters
                        </button>
                    </div>
                )}
